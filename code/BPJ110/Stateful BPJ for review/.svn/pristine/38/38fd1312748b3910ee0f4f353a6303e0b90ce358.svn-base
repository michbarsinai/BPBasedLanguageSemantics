...
import static bp.BProgramControls.bpjPackage;
import static bp.BProgramControls.continuationMode;
import static bp.BProgramControls.debugMode;
import static bp.BProgramControls.globalRunMode;
import static bp.BProgramControls.iterativeMode;
import static bp.BProgramControls.javaflowInstrumentation;
import static bp.BProgramControls.logMode;
...
import bp.exceptions.BPJInvalidParameterException;
...
import bp.state.BPState;
import bp.state.Context;
import bp.state.LivenessVerification;
...
public class BProgram implements Serializable {
	 ...
	 */
	/**
	 * A set containing all the b-threads in the system. A b-thread adds
	...
	// that was triggered.
	transient  BThread bThreadOfLastEvent;
	 ...
	 * the b-thread that decremented it from one to zero, awakes other
	 * b-threads and sets the counter to the number of b-threads that it
	 * awakes (the number of b-threads that are waiting to the next event)
	 */
	...
	public BThread add(BThread bt, Double priority) throws BPJDuplicatePrioritiesException, BPJDuplicateBthreadException {
		synchronized (getAllBThreads()) {
			...
			{
				JavaThreadForBThread newlyCreatedThread = new JavaThreadForBThread("java thread for"+bt.getName(), bt);
				bt.setThread(newlyCreatedThread );
			}
	...
	{
		if (threadToBThreadMap.get(thread) != null) {
			return true;
		}
		return false;
	}
	...
	public void bplog( String message, Throwable t ) {
		if ( logMode ) {
	...
	public void bSync(RequestableInterface requestedEvents, EventSetInterface watchedEvents, EventSetInterface blockedEvents) throws BPJException {
		...
		if (bt == null) {
			BPJMissingBThreadException e = new BPJMissingBThreadException(currentThread);
			e.printStackTrace();
			throw e;
		}
		synchronized (bt) {
			...
			synchronized (getAllBThreads()) {
				...
				// Remove this b-thread from the count of busy b-threads
				// because
				// it
				// is about to sleep or choose the next event
				busyBThreads--;

				// If end of b-thread, remove the b-thread from allBThreads
				// and
				...
				if (bt.getThread() == null) {
					...
					// be-program "finished" - more dynamic b-threads
					// may be added.
			...
			if (!continueImmediately) {
				...
				// Don't count this b-thread as running for concurrency
				// purposes
				if (concurrency.control) {
	 ...
	 */
	void chooseNextEvent() throws BPJRequestableSetException {
...
//		System.out.println("Calling prunedAtNextVerificationState");
	...
	// Remove the running Java thread that had added itself as a b-thread
	// during
	// run
	...
	// GW: Get all waited-for events when program is idle
	...
	public Collection<Event> getRequestedBlockedEvents() {
		Collection<Event> blocked = new ArrayList<Event>();
		for (BThread bt : allBThreads.values()) {
			Iterator<RequestableInterface> it = bt.requestedEvents.iterator();
			while (it.hasNext()) {
				RequestableInterface req = it.next();
				if (req.isEvent()) {
					Event e = (Event) req;
					for (BThread other : allBThreads.values()) {
						if (other.blockedEvents.contains(e)) {
							blocked.add(e);
						}
					}
				}
