...
import static bp.BProgramControls.continuationMode;
import static bp.BProgramControls.estimatedStates;
import static bp.BProgramControls.globalDFS;
import static bp.BProgramControls.globalDisableStateHashing;
import static bp.BProgramControls.globalRunSubtree;
import static bp.BProgramControls.globalSafetyCheck;
import static bp.BProgramControls.suppressDeadlock;
import static bp.BProgramControls.globalLogBacktracking;
...
import bp.BThread;
import bp.Event;
import bp.exceptions.VerificationFailedException;
...
public class BPState {
	...
	/** The states of the relevant b-threads */
	...
	/**
	 * Start a be-program and return when all b-threads arrive at at a bsync.
	...
	static BPState startWith(BProgram bp) throws IOException {
		...
		// Progress all b-threads to their next be-sync and store their new
		// state.
		...
		// System.out.println("bp.getAllEnabledEvents()=" + bps.enabledEvents);
	...
	/**
	 * Continue a system run from a given state until all b-threads arrive at
	 * the next bsync.
	 * 
	 * @param oldbps
	 ...
	 * @param event
	 *            The next event.
	 * @return The state on the next arrival of all b-threads to a bsync.
	...
	static BPState continueWith(BPState oldbps, Event event) throws IOException, ClassNotFoundException {
		...
		// System.out.println("bp.getAllEnabledEvents()=" +
	 ...
	 * 
	 * @param bp
	 *            A BProgram with all b-threads added.
	 * @throws IOException
	 * @throws ClassNotFoundException
	 */
	...
	public static void verifyAllRuns(BProgram bp) throws IOException, ClassNotFoundException {
		...
		try {
			...
			JOptionPane.showMessageDialog(null, "Verification successful", "BPJ Model Checking Results", JOptionPane.INFORMATION_MESSAGE);
			...
			JOptionPane.showMessageDialog(null, "Verification failed:\n" +
					ex.getShortMessage(), "BPJ Model Checking Results", JOptionPane.INFORMATION_MESSAGE);
		}
		// Print results
...
//			System.out.println("Duration (msec)=" + time);
	...
	/**
	 * Returns a short trace, with only the triggering events, to a given BPState.
	 * @param bps The given BPState.
	 */
	static String getShortTrace(BPState bps){
		BPState temp = bps;
		String trace = "";
		while (temp.triggeringEvent != null){
			trace = temp.triggeringEvent.getName() /*+ "->" + temp */ +  "\n" + trace;
			temp = temp.predecessorBPS;
		}
		return trace;
	}
		...
		while (!queue.isEmpty()) {
			...
			for (Event e : bps.enabledEvents) {
				...
				if (newState.isBad()) {
					...
					VerificationFailedException verificationFailesException = new VerificationFailedException(getTrace(newState));
					verificationFailesException.setShortMessage(getShortTrace(newState));
					throw verificationFailesException;/*new VerificationFailedException(getTrace(newState));*/
				} 
				//else {
	...
	/**
	 * Check if the state is bad by checking if one of its b-threads is in a
	 * bad state
	 * 
	...
	/** States are identified by the constituent b-thread states */
	public int hashCode() {
		if (hashCode != 0)
			return hashCode;
		else
