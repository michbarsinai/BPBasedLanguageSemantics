...
import static bp.BProgramControls.globalShallow;
...
/**
 *
 * A class for capturing b-thread states.
 *
 */
public class BTState {
	...
	/** The b-thread whose state is captured */
	BThread bt;
	...
	/**
	 * Object that stores the label that the corresponding b-thread assigned to
	 * the state. This object is null until the b-thread calls
	...
	/**
	 * Run a b-thread to its first be-sync.
	 *
	 * @param bt
	 *            A b-thread that was not yet started.
	...
	static BTState startWith(BThread bt) throws IOException {
		...
		// Store a pointer to the b-thread in the state object.
		st.bt = bt;

		// Run the b-thread to its first be-sync.
		...
		// Return an object representing the state of the b-thread at its
		// first be-sync.
		return st;
	}

	/**
	 * Continue the run of a b-thread to its next be-sync.
	 *
	 * @param oldst
	 *            A state of a b-thread.
	 * @return The state of the b-thread at its next be-sync.
	...
	static BTState cachedDisabledcontWith(BTState oldst) throws IOException, ClassNotFoundException {
		...
		// Copy the reference to the b-thread and its id.
		newst.bt = oldst.bt;

		// Run the b-thread to its next be-sync.
	...
	static BTState cacheEnabledcontWith(BTState oldst) throws IOException, ClassNotFoundException {
		...
		if (cacheKey != null && cacheResult != null) {
			...
			// Run the b-thread to its net be-sync
	...
	/**
	 * Restore the state of the b-thread without running it to its next be-sync..
	 *
	 * @param st
	 *            An object representing the state of a b-thread.
	 * @throws ClassNotFoundException
	 * @throws IOException
	 */
